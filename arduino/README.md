...
void setup()
{
  ...
  MyTim->setMode(2, TIMER_OUTPUT_COMPARE, LED); // configure the timer mode
...
void setup()
{
  ...
  MyTim->setMode(2, TIMER_OUTPUT_COMPARE, LED); // configure the timer mode without output to any pin
  MyTim->setOverflow(1000000, MICROSEC_FORMAT); // configure the timer period to 1 second
  MyTim->attachInterrupt(changeLEDState);       // indicate the name of the ISR to execute
  MyTim->resume();                              // start the timer
...
Let's repeat the previous exercise: toggle the LED mode from off to blinking and vice versa with the button. This time, all through interrupts and timers. **There must be no code in the `loop` function**.
